...
def parse_entry:
  ...
  | .key |= ascii_downcase
  | if .key | within($keys | map(ascii_downcase)) | not then break $out
  else . end
  # TODO: Change "type" key (used for backward compatibility with Ananicy) into
  # "class" or "family" key in cache
  | if .key | within(["origin", "name", "type","cgroup"]) then .
  elif .key | within(cgroup_keys | map(ascii_downcase)) then
  ...
  elif .key | within(type_keys - cgroup_keys) then
  ...
  elif .key | within(rule_keys - type_keys) then
